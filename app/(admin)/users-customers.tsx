...
export default function AdminCustomersUsersScreen() {
  const [users, setUsers] = useState<CustomerUser[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);

  const fetchUsers = useCallback(async () => {
    const data = await apiFetch("/admin/users?role=CUSTOMER");
    setUsers(data.users || []);
  }, []);
      ...
      ListHeaderComponent={
        <>
          ...
          <Text style={styles.subtitle}>
            All customer accounts currently registered on Safirisha.
          </Text>
