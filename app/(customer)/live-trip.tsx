...
function getCustomerStatusHint(status?: TripStatus, paymentMethod?: PaymentMethod | null, paymentStatus?: PaymentStatus | null) {
  switch (status) {
...
function getProgressStep(status?: TripStatus) {
  switch (status) {
    ...
    case "ACCEPTED": case "DRIVER_EN_ROUTE": case "ARRIVED_PICKUP": return 2;
    case "PICKUP_CONFIRMED": case "IN_TRANSIT": case "ARRIVED_DROPOFF": return 3;
    case "DELIVERY_CONFIRMED": case "PAYMENT_PENDING": return 4;
...
export default function LiveTripScreen() {
  ...
  const statusHint = useMemo(() => getCustomerStatusHint(currentStatus, currentPaymentMethod, currentPaymentStatus), [currentStatus, currentPaymentMethod, currentPaymentStatus]);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (!fetchedTrip) {
        Alert.alert("Live trip unavailable", store.getState().error || "Failed to load live trip details.");
        return;
      }

      if (fetchedTrip.status === "CANCELLED") {
        closeFlow(fetchedTrip.expiredAt ? "Search expired" : "Trip cancelled", fetchedTrip.expiredAt ? "No driver accepted this request in time." : "This trip has been cancelled.", "/(customer)/(tabs)");
        return;
      }

      const assignedDriver = fetchedTrip.assignedDriver;
      if (assignedDriver && typeof assignedDriver.currentLat === "number" && typeof assignedDriver.currentLng === "number") {
    ...
    const onTripStatusUpdated = (payload: TripStatusUpdatedPayload) => {
      ...
      if (currentTrip) {
        useTripStore.getState().setCurrentTrip({
          ...
          paymentMethod: payload.paymentMethod !== undefined ? payload.paymentMethod : currentTrip.paymentMethod,
          paymentStatus: payload.paymentStatus !== undefined ? payload.paymentStatus : currentTrip.paymentStatus,
        } as Trip);
      }
    };
  ...
  const handleConfirmPickup = () => {
    ...
    Alert.alert("Confirm pickup", "Confirm only after the driver has received your goods at pickup.", [
      ...
      { text: "Confirm pickup", onPress: async () => {
        ...
        if (useTripStore.getState().error) Alert.alert("Confirmation failed", useTripStore.getState().error || "Could not confirm pickup.");
      }},
    ]);
  };
  ...
  const handleConfirmDelivery = () => {
    ...
    Alert.alert("Confirm delivery", "Confirm only after you have received the goods at drop-off.", [
      ...
      { text: "Confirm delivery", onPress: async () => {
        ...
        if (useTripStore.getState().error) Alert.alert("Confirmation failed", useTripStore.getState().error || "Could not confirm delivery.");
      }},
    ]);
  };
  ...
  const selectCash = async () => {
    ...
    if (useTripStore.getState().error) Alert.alert("Payment method failed", useTripStore.getState().error || "Could not choose cash payment.");
  };
  ...
  if (!safeTripId) {
    return (
      <SafeAreaView style={styles.centerScreen}>
        ...
        <Text style={styles.errorText}>We could not open this live trip because the trip ID is missing.</Text>
  ...
  const routeLineCoordinates = driverLocation
    ? [
        ...
        {
          latitude: (currentStatus === "ACCEPTED" || currentStatus === "DRIVER_EN_ROUTE" || currentStatus === "ARRIVED_PICKUP") ? trip.pickupLat : trip.dropoffLat,
          longitude: (currentStatus === "ACCEPTED" || currentStatus === "DRIVER_EN_ROUTE" || currentStatus === "ARRIVED_PICKUP") ? trip.pickupLng : trip.dropoffLng,
        },
      ]
    : [];
  ...
  return (
    <View style={styles.container}>
      <MapView ref={mapRef} style={styles.map} initialRegion={mapRegion} showsUserLocation={false} showsMyLocationButton={false}>
        <Marker coordinate={{ latitude: trip.pickupLat, longitude: trip.pickupLng }} title="Pickup" description={trip.pickupAddress}>
          <View style={styles.pickupMarker}><Ionicons name="arrow-up" size={16} color="#ffffff" /></View>
        </Marker>
        <Marker coordinate={{ latitude: trip.dropoffLat, longitude: trip.dropoffLng }} title="Drop-off" description={trip.dropoffAddress}>
          <View style={styles.dropoffMarker}><Ionicons name="location" size={16} color="#ffffff" /></View>
        </Marker>
        {routeLineCoordinates.length === 2 ? (
          <Polyline coordinates={routeLineCoordinates} strokeWidth={4} strokeColor="#111827" />
        ) : null}
        {driverLocation ? (
          <Marker coordinate={{ latitude: driverLocation.lat, longitude: driverLocation.lng }} title="Driver" description={displayDriverName}>
            <View style={styles.driverMarkerWrap}>
              <View style={styles.driverMarkerPulse} />
              <View style={styles.driverMarker}>
                <MaterialCommunityIcons name={displayVehicleIcon as any} size={18} color="#fff" />
              </View>
            </View>
          </Marker>
        ) : null}
      ...
      <SafeAreaView pointerEvents="box-none" style={styles.overlayContainer}>
        <View style={styles.topBar}>
          ...
          <TouchableOpacity style={styles.topActionButton} onPress={fitMapToPoints}>
        ...
        <View style={styles.bottomSheet}>
          ...
          <View style={styles.statusRow}>
            ...
            <View style={[styles.statusPill, { backgroundColor: statusPillColors.bg }]}>
              <Text style={[styles.statusPillText, { color: statusPillColors.text }]}>{currentStatusLabel}</Text>
            </View>
          </View>
          ...
          {currentStatus !== "CANCELLED" ? (
            <View style={styles.progressCard}>
              <View style={styles.progressRow}>
                {[1, 2, 3, 4, 5].map((step) => (
                  <View key={step} style={[styles.progressDot, step <= progressStep ? styles.progressDotActive : styles.progressDotInactive]} />
                ))}
              </View>
          ...
          <View style={styles.driverCard}>
            <View style={styles.driverHeader}>
              <View style={styles.avatar}>
                <MaterialCommunityIcons name={displayVehicleIcon as any} size={22} color="#111827" />
              </View>
              <View style={styles.driverTextWrap}>
                <Text style={styles.driverName}>{displayDriverName}</Text>
                <Text style={styles.driverMeta}>{displayVehicleType} • Plate: {displayPlateNumber}</Text>
              </View>
          ...
          <View style={styles.routeCard}>
            ...
            <View style={styles.routeMetaRow}>
              <View style={styles.routeMetaCard}>
                ...
                <Text style={styles.routeMetaValue}>KES {Number(trip.estimatedPrice || 0).toLocaleString()}</Text>
              </View>
              <View style={styles.routeMetaCard}>
          ...
          {currentStatus === "ARRIVED_PICKUP" && (
            <TouchableOpacity style={[styles.actionButton, isSubmitting && styles.buttonDisabled]} onPress={handleConfirmPickup} disabled={isSubmitting}>
              {isSubmitting ? <ActivityIndicator color="#fff" /> : <><Ionicons name="checkmark-circle-outline" size={18} color="#fff" /><Text style={styles.actionButtonText}>Confirm Pickup Handover</Text></>}
            </TouchableOpacity>
          )}

          {currentStatus === "ARRIVED_DROPOFF" && (
            <TouchableOpacity style={[styles.actionButton, isSubmitting && styles.buttonDisabled]} onPress={handleConfirmDelivery} disabled={isSubmitting}>
              {isSubmitting ? <ActivityIndicator color="#fff" /> : <><Ionicons name="checkmark-done-circle-outline" size={18} color="#fff" /><Text style={styles.actionButtonText}>Confirm Delivery Received</Text></>}
            </TouchableOpacity>
          )}
          ...
          {showPaymentOptions && (
            <View style={styles.paymentCard}>
              ...
              <TouchableOpacity style={[styles.paymentOptionButton, isSubmitting && styles.buttonDisabled]} onPress={selectCash} disabled={isSubmitting}>
                {isSubmitting ? <ActivityIndicator color="#111827" /> : <><Ionicons name="cash-outline" size={18} color="#111827" /><Text style={styles.paymentOptionText}>Pay with Cash</Text></>}
              </TouchableOpacity>
              <TouchableOpacity style={[styles.paymentOptionPrimary, isSubmitting && styles.buttonDisabled]} onPress={initiateMpesa} disabled={isSubmitting}>
                {isSubmitting ? <ActivityIndicator color="#fff" /> : <><Ionicons name="phone-portrait-outline" size={18} color="#fff" /><Text style={styles.paymentOptionPrimaryText}>Pay with M-Pesa</Text></>}
              </TouchableOpacity>
            </View>
          )}
          ...
          {showPaymentPendingInfo && (
            <View style={styles.paymentPendingCard}>
              ...
              <Text style={styles.paymentPendingText}>Method: {currentPaymentMethod || "Not selected"}</Text>
              <Text style={styles.paymentPendingText}>Status: {currentPaymentStatus}</Text>
            </View>
          )}

          {canCustomerCancel(trip.status) && (
            <TouchableOpacity style={[styles.cancelButton, isSubmitting && styles.buttonDisabled]} onPress={handleCancelTrip} disabled={isSubmitting}>
...
const styles = StyleSheet.create({
  ...
  sheetSubtitle: { marginTop: 5, color: "#64748b", fontWeight: "700" },
  statusHint: { marginTop: 8, color: "#475569", lineHeight: 20, fontWeight: "600" },
  statusPill: { borderRadius: 999, paddingHorizontal: 10, paddingVertical: 8, alignSelf: "flex-start" },
  statusPillText: { fontSize: 12, fontWeight: "800" },
  progressCard: { backgroundColor: "#f8fafc", borderRadius: 18, padding: 14, borderWidth: 1, borderColor: "#e5e7eb", marginBottom: 12 },
  ...
  driverCard: { backgroundColor: "#f8fafc", borderRadius: 18, padding: 14, borderWidth: 1, borderColor: "#e5e7eb", marginBottom: 12 },
  driverHeader: { flexDirection: "row", alignItems: "center" },
  avatar: { width: 46, height: 46, borderRadius: 16, backgroundColor: "#e5e7eb", justifyContent: "center", alignItems: "center", marginRight: 12 },
  ...
  routeLabel: { color: "#64748b", fontSize: 12, fontWeight: "700", marginBottom: 4 },
  routeValue: { color: "#111827", fontWeight: "800", lineHeight: 20 },
  routeMetaRow: { flexDirection: "row", gap: 10, marginTop: 6 },
  routeMetaCard: { flex: 1, backgroundColor: "#f8fafc", borderRadius: 14, padding: 12, borderWidth: 1, borderColor: "#e5e7eb" },
  routeMetaLabel: { color: "#64748b", fontSize: 12, fontWeight: "700", marginBottom: 4 },
  routeMetaValue: { color: "#111827", fontWeight: "800" },
  paymentCard: { backgroundColor: "#f8fafc", borderRadius: 18, padding: 14, borderWidth: 1, borderColor: "#e5e7eb", marginBottom: 12 },
  paymentTitle: { color: "#111827", fontWeight: "900", fontSize: 16, marginBottom: 10 },
  paymentOptionButton: { backgroundColor: "#fff", borderRadius: 14, paddingVertical: 14, borderWidth: 1, borderColor: "#d1d5db", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 10 },
  paymentOptionText: { color: "#111827", fontWeight: "800" },
  paymentOptionPrimary: { backgroundColor: "#111827", borderRadius: 14, paddingVertical: 14, borderWidth: 1, borderColor: "#111827", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 },
  paymentOptionPrimaryText: { color: "#fff", fontWeight: "800" },
  paymentPendingCard: { backgroundColor: "#faf5ff", borderRadius: 18, padding: 14, borderWidth: 1, borderColor: "#ddd6fe", marginBottom: 12 },
  paymentPendingTitle: { color: "#6d28d9", fontWeight: "900", marginBottom: 8, fontSize: 16 },
  paymentPendingText: { color: "#5b21b6", fontWeight: "700", marginTop: 4, lineHeight: 19 },
  pickupMarker: { width: 34, height: 34, borderRadius: 999, backgroundColor: "#16a34a", justifyContent: "center", alignItems: "center", borderWidth: 2, borderColor: "#ffffff" },
  dropoffMarker: { width: 34, height: 34, borderRadius: 999, backgroundColor: "#dc2626", justifyContent: "center", alignItems: "center", borderWidth: 2, borderColor: "#ffffff" },
  driverMarkerWrap: { alignItems: "center", justifyContent: "center" },
  driverMarkerPulse: { position: "absolute", width: 44, height: 44, borderRadius: 999, backgroundColor: "rgba(17,24,39,0.18)" },
  driverMarker: { width: 36, height: 36, borderRadius: 999, backgroundColor: "#111827", justifyContent: "center", alignItems: "center", borderWidth: 2, borderColor: "#ffffff" },
  actionButton: { backgroundColor: "#111827", borderRadius: 16, paddingVertical: 15, borderWidth: 1, borderColor: "#111827", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 12 },
  actionButtonText: { color: "#fff", fontWeight: "800", fontSize: 15 },
  cancelButton: { backgroundColor: "#fff1f2", borderRadius: 16, paddingVertical: 15, borderWidth: 1, borderColor: "#fecdd3", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 },
  cancelButtonText: { color: "#b91c1c", fontWeight: "800", fontSize: 15 },
  buttonDisabled: { opacity: 0.7 },
  centerScreen: { flex: 1, backgroundColor: "#fff", justifyContent: "center", alignItems: "center", padding: 20 },
  loadingTitle: { marginTop: 14, fontSize: 18, fontWeight: "900", color: "#111827" },
  loadingText: { marginTop: 6, color: "#64748b", textAlign: "center" },
  errorTitle: { fontSize: 18, fontWeight: "900", color: "#111827", marginBottom: 8 },
  errorText: { color: "#64748b", textAlign: "center", marginBottom: 16 },
  primaryButton: { backgroundColor: "#111827", paddingHorizontal: 18, paddingVertical: 14, borderRadius: 14 },
